...
Muse.Redirect.redirect = function (a, b, c) {
  if (Muse.Redirect.readCookie("inbrowserediting") != "true") {
    var d, g = Muse.Redirect.readCookie("devicelock");
    g == "phone" && c ? d = c : g == "tablet" && b && (d = b);
    if (g != a && !d)
      if (window.matchMedia) window.matchMedia("(max-device-width: 415px)").matches && c ? d = c : window.matchMedia("(max-device-width: 960px)").matches && b && Muse.Redirect.Touch && (d = b);
      else {
        var a = Math.min(screen.width, screen.height) / (window.devicePixelRatio || 1),
          g = window.screen.systemXDPI || 0,
          f = window.screen.systemYDPI || 0,
          g = g > 0 && f > 0 ? Math.min(screen.width / g, screen.height / f) : 0;
        (a <= 370 || g != 0 && g <= 3) && c ? d = c : a <= 960 && b && Muse.Redirect.Touch && (d = b)
      }
    if (d) document.location = d, document.write('<style type="text/css">body {visibility:hidden}</style>')
  }
};;
(function () {
    ...
    if (-1 != a) {
      ...
      for (var a = document.getElementsByTagName("meta"), b = 0, c = a.length; b < c; b++) {
        ...
        if ("generator" == d.getAttribute("name")) {
          "2015.1.0.342" != d.getAttribute("content") && Muse.assets.outOfDate.push("museredirect.js");
          break
        }
      }
    }
